...
declare @startDate date = '03/20/2013'
declare @endDate date = '04/19/2013' 
...
begin
	...
	if(@Supervisor != '%') delete from @TC where Supervisor <> @Supervisor
		...
		select distinct * from @TC order by Supervisor , OrgTechCode 
		
		
		
		/*Compleed Jobs Count*/
	/*	select sup.Supervisor,  count(tjs.WoNumber) as JobsCompleted from
