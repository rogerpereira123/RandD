...
t.NewTier = 
(case 
 when  t.PPH < 5.75 then 'Tier 1'
 when  t.PPH >= 8.50 then 'Tier 6'
...
case 
--Failed only in TC30
when t.TC30 > 5.5 and t.CSAT >= 9.7 and t.SHSPerWO >= 10 and t.PPH >= 5.75 then 'Tier ' + CONVERT(varchar, CAST(SUBSTRING(t.NewTier , len(t.NewTier) , 1) as int) - 1)
--Failed only in CSAT
when t.TC30 <= 5.5 and t.CSAT < 9.7 and t.SHSPerWO >= 10 and t.PPH >= 5.75 then 'Tier ' + CONVERT(varchar, CAST(SUBSTRING(t.NewTier , len(t.NewTier) , 1) as int) - 1)
--Failed only in SHSPerWO
when t.TC30 <= 5.5 and t.CSAT >= 9.7 and t.SHSPerWO < 10 and t.PPH >= 5.75 then 'Tier ' + CONVERT(varchar, CAST(SUBSTRING(t.NewTier , len(t.NewTier) , 1) as int) - 1)
--Failed in more than one parameter
when t.TC30 > 5.5 or t.CSAT < 9.7 or t.SHSPerWO < 10 or t.PPH < 5.75 then 'Tier 1'
--else keep the same tier
else NewTier
end
from @T t 
where t.NewTier like 'Tier%' 
and t.NewTier not in ('Tier 1' , 'Tier 0') and t.CurrentTierComments = ''
