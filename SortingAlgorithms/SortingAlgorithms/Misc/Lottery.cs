...
This problem statement is the exclusive and proprietary property of TopCoder, Inc. Any unauthorized use or reproduction of this information without the prior written consent of TopCoder, Inc. is strictly prohibited. (c)2003, TopCoder, Inc. All rights reserved. 
    ...
    class Lottery
    {
        long getValidTicketsFromRule(string s)
        {
            string[] names = s.Split(new string[1] { ": " }, StringSplitOptions.None);
            string[] rules = names[1].Trim().Split(new string[1] { " " }, StringSplitOptions.None);

            int choices = Convert.ToInt32(rules[0]);
            int blanks = Convert.ToInt32(rules[1]);
            bool sorted = rules[2] == "T" ? true : false;
            bool unique = rules[3] == "T" ? true : false;
            long result = Convert.ToInt64(Math.Pow(choices, blanks));
            if (unique)
            {
                result = choices;
                var n = choices;
        ...
        public string[] sortByOdds(string[] rules)
        {
            ...
            {
                var n = getValidTicketsFromRule(r);
                Console.WriteLine(r + " = " + n);
                double odds = 1.0 / n;
                d.Add(r,odds);
            }
            var result = from entry in d orderby entry.Value descending select entry.Key.Remove(entry.Key.IndexOf(":"));
